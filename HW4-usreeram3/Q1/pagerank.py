...
"""
def author():                                                                                             
        return 'usreeram3' # replace sroutray3 with your Georgia Tech username.                                                                                             
                                                                                              
def gtid():                                                                                               
    return 903452513 # replace with your GT ID number      
...
class PageRank:
    ...
    def calculate_node_degree(self):
        for source,target in self.read_edge_file(self.edge_file):
            #max node id
            if self.max_node_id < max(source,target):
    ...
    def run_pagerank(self, node_weights,  damping_factor=0.85, iterations=10):
        ...
        for it in range(iterations):
            ...
            for target in range(self.max_node_id+1):
                new_scores[target] = ((1 - damping_factor) * node_weights[target]) + new_scores[target]
